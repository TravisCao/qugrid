...
工程单位、同一次运行给出的经典参照、诚实的成功概率——第一屏就全部可见。QuGrid 服务两类研究者:想研究量子算法、但**不想离开本领域的工具、单位和证据标准**的电力系统研究者;以及想要一批经得起电力工程师评审的电网问题的量子计算研究者。
...
3. **求解器层**运行在纯 NumPy 态矢量内核上——QAOA 及其[文献变体](https://traviscao.github.io/qugrid/methods/)(热启动、XY 混合器、角度迁移)、VQE、HHL、VQLS、量子核方法、量子玻尔兹曼机——**不依赖任何量子 SDK**;旁边就是每个结论都必须面对的经典基线:精确枚举、固定种子模拟退火、禁忌搜索、并行回火、随机采样加贪心修复。同一个问题对象可一键导出到 Qiskit、D-Wave Ocean、PennyLane,也可以直接经由它们求解(`solver="dimod-exact"`、`"dwave-sa"`、`"qiskit-qaoa"`),返回同一个 `Result`。

所有求解器返回同一个 `Result` 对象:解码后的工程答案、**原始约束**(而非惩罚项代理)的可行性、同一次运行中算出的与经典参照的 `gap()`、即使无噪声的理想硬件也要面对的成功概率、以及资源账单(量子比特数、电路评估次数、运行时间)。

## 六十秒证据

WSCC 9 节点系统的主动解列。QUBO 精确最优解断开 2 条线路,两个孤岛的功率不平衡为 +10.3 / −5.0 MW;深度 2 的 QAOA 找到同一方案:

<p align="center"><img alt="WSCC 9 节点系统解列结果" src="docs/assets/hero_islanding.png" width="560"></p>
...
| 主动解列,WSCC 9 节点 | 精确 = 模拟退火 = QAOA(p=2),gap 为 0 | 精确枚举 |
| PMU 配置,9 节点 / 14 节点 | 模拟退火找到 3 / 4 台,全网可观测 | 精确最少台数:3 / 4 |
| 量子核方法,N-1 安全校核 | 带宽调好时测试准确率 1.00,调错时 0.50 | RBF 核:1.00 |
| 不等式编码,PMU 配置(PJM 5 节点) | 非平衡惩罚:5 个量子比特,P(optimum) 0.54;松弛位编码:15 个量子比特,0.001 | 两种编码的精确解一致:2 台 PMU |
| 热启动 QAOA,解列问题 p=1 | 以模拟退火答案热启动,P(optimum) 从 0.009 升到 0.128 | 精确枚举,同一次运行 |
...
[`examples/`](examples/) 中的十三个单文件研究,风格取自 CleanRL:自包含、固定随机种子、笔记本电脑上几分钟跑完、自带断言校验、经典基线写在同一个文件里。复制一个、换上你的算例文件,就得到一篇论文实验部分的骨架。

| # | 研究 | # | 研究 |
|---|---|---|---|
| 01 | HHL 解直流潮流:误差解剖 | 08 | 量子核 vs RBF(N-1 校核) |
| 02 | QAOA 解机组组合,诚实的成功概率 | 09 | 量子玻尔兹曼机生成风电场景 |
| 03 | WSCC 9 节点主动解列 | 10 | 混合牛顿-拉夫逊 + 变分线性求解器 |
| 04 | PMU 配置:松弛位不等式编码 | 11 | 跨库基准:dimod、Ocean、Qiskit |
| 05 | 离散化的代价:编码误差 vs 求解误差 | 12 | 约束处理:松弛位 vs 非平衡惩罚 vs 增广拉格朗日 |
| 06 | QAOA 深度研究(解列问题) | 13 | QAOA 变体:热启动、XY 混合器、角度迁移 |
| 07 | 七求解器多种子基准 → LaTeX 表格 | | |
...
- **Qiskit Optimization、OpenQAOA、D-Wave Ocean** 处理不带电网语义的抽象优化模型。QuGrid 负责它们之前的事(可信的电网问题构建、有文档的惩罚权重、离散化核算)和之后的事(解码回兆瓦、真实约束的可行性、领域标准基线)——并可一键导出到这三者。
