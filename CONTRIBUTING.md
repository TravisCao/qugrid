...
2. **A zoo script.** A single self-validating file in `examples/` following the contract in [the zoo page](https://traviscao.github.io/qugrid/zoo/): docstring (problem / why quantum / expected output), classical baseline in the same run, final assertion, one or two figures.
...
- **Honest benchmarking is non-negotiable.** Contributions that compare a quantum method only against weak baselines, or state advantage claims the run does not measure, will be asked to revise. The [six rules](https://traviscao.github.io/qugrid/honest-benchmarking/) are the review checklist.
